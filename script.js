...
const SESSION_STORAGE_KEY = "flowtrack-session";
...
function renderCurrentUser() {
  ...
  els.currentUserName.textContent = currentUser ? currentUser.name : "";
}
...
function registerUser({ name, email, password }) {
  ...
  state = loadStateForUser(newUser.id);
  normalizeState();
  applyTheme();
  renderAll();
  renderCurrentUser();
...
function bindStaticControls() {
  ...
  els.showLoginBtn?.addEventListener("click", () => syncAuthTabs("login"));
  els.showRegisterBtn?.addEventListener("click", () => syncAuthTabs("register"));
